...
public class HttpMessageProcessTemplate extends AbstractTransportMessageProcessTemplate<HttpMessageReceiver, HttpConnector> implements RequestResponseFlowProcessingPhaseTemplate, ThrottlingPhaseTemplate, EndPhaseTemplate
{
    ...
    protected MuleMessage createMessageFromSource(Object message) throws MuleException
    {
        ...
        processRemoteClientAddress(muleMessage); //Added
    ...
    /** 
     * Added. RemoteClientAddress can not be used if X-Forwarded-For is used 
     * @see processRemoteAddresses
     */
    public void processRemoteClientAddress(MuleMessage muleMessage) {
        muleMessage.setProperty(X_MULE_REMOTE_CLIENT_ADDRESS,
        		getHttpServerConnection().getRemoteClientAddress(), PropertyScope.INBOUND);
    }
